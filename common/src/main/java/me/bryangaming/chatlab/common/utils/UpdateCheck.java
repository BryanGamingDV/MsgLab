 ...
 * class are accessed, {@link #init(PluginBaseWrapper, int)} must be invoked by the plugin,
 * preferrably in its onEnable method, though that is not a
 * requirement.
 * <p>
...
public final class UpdateCheck {
    ...
    private UpdateCheck(PluginBaseWrapper plugin, int pluginID, VersionScheme versionScheme) {
        this.plugin = plugin;
     ...
     * immediately as an HTTP GET request is published to the SpiGet ChatApiImpl.
     *
     ...
     */
    public static UpdateCheck init(PluginBaseWrapper plugin, int pluginID, VersionScheme versionScheme) {
     ...
     * @return the UpdateChecker instance
     */
    public static UpdateCheck init(PluginBaseWrapper plugin, int pluginID) {
        return init(plugin, pluginID, VERSION_SCHEME_DECIMAL);
    }

    /**
     * Get the initialized instance of UpdateChecker. If {@link #init(PluginBaseWrapper, int)} has not yet been
    ...
    /**
     * Check whether the UpdateChecker has been initialized or not (if {@link #init(PluginBaseWrapper, int)}
    ...
    public static enum UpdateReason {
        ...
        /**
         * A successful connection to the SpiGet ChatApiImpl could not be established.
         */
        COULD_NOT_CONNECT,
        ...
        /**
         * A 401 error was returned by the SpiGet ChatApiImpl.
         */
        UNAUTHORIZED_QUERY,
    ...
    /**
     * Represents a result for an update query performed by {@link UpdateCheck#requestUpdateCheck()}.
     */
    public final class UpdateResult {
        ...
        { // An actual use for initializer blocks. This is madness!
            UpdateCheck.this.lastResult = this;
        }
