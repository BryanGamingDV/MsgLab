...
public class SocialSpyManager implements Option {
    ...
    public void toggleOption(UUID uuid) {
        ...
        if (userData.isSocialSpyMode()) {
            ...
            status = pluginService.getFiles().getMessagesFile().getString("socialspy.player.variable-off");
            return;
        }

        userData.toggleSocialSpy(true);
        status = pluginService.getFiles().getMessagesFile().getString("socialspy.player.variable-on");
    }
