...
public class SenderManager {
    ...
    public SenderManager(PluginService pluginService) {
        ...
        this.supportManager = pluginService.getSupportManager();
    ...
    public boolean hasUtilsPermission(Player player, String path) {
        ...
        if (path == null) {
            ...
            if (!configFile.getString("config-version", "1.0").equalsIgnoreCase(pluginService.getPlugin().getDescription().getVersion())) {
    ...
    public boolean hasPermission(CommandSender sender, String command, String path) {
        if (!(sender instanceof Player)) {
            return true;
        }

        return hasPermission((Player) sender, command, path);
    }

    public boolean hasPermission(Player player, String command, String path) {

        Logger logger = pluginService.getPlugin().getLogger();
        String permission = configFile.getString("modules." + command + ".permissions." + path);
    ...
    public boolean isVanished(Player player){
        ...
        if (supportManager.getVanishSupport().isVanished(player)){
            return true;
        }
