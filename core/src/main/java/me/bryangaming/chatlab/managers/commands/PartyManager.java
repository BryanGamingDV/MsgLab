...
public class PartyManager {
    ...
    public void createParty(Player playerLeader) {
        ...
        if (userData.getPartyID() > 0){
            senderManager.sendMessage(playerLeader, messagesFile.getString("party.error.already-have"));
        ...
        serverData.createParty(configFile.getInt("modules.party.config.max-players"), partyID, playerLeader.getUniqueId());
        serverData.getParty(partyID).invitePlayer(playerLeader.getUniqueId());
        partyID++;

        senderManager.sendMessage(playerLeader, messagesFile.getString("party.created"));
    ...
    public void deleteParty(Player player) {
        ...
        if (userData.getPartyID() < 1) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.no-party"));
        ...
        if (partyData.getPlayerLeader() != player.getUniqueId()) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.no-perms.delete"));
            senderManager.playSound(player, SoundEnum.ERROR);
            return;
        }
        ...
        senderManager.sendMessage(player, messagesFile.getString("party.deleted"));
    ...
    public void joinParty(Player player, Player leader) {
        ...
        if (playerData.getPartyID() > 1) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.already-joined"));
            senderManager.playSound(player, SoundEnum.ERROR);
            return;
        }
        ...
        if (partyData.getPlayerLeader() != leader.getUniqueId()) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.no-leader"));
            senderManager.playSound(player, SoundEnum.ERROR);
            return;
        }

        if (partyData.isFull()){
            senderManager.sendMessage(player, messagesFile.getString("party.error.invite.max-players")
        ...
        if (partyData.isPrivate()) {
            if (!partyData.isInvited(player.getUniqueId())) {
                senderManager.sendMessage(player, messagesFile.getString("party.error.private"));
                senderManager.playSound(player, SoundEnum.ERROR);
                return;
            }
        }
        ...
        senderManager.sendMessage(player, messagesFile.getString("party.join"));
    ...
    public void invitePlayer(Player player, Player target) {
        ...
        if (targetData.getPartyID() > 0) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.target.already-have")
        ...
        if (partyData.isInvited(target.getUniqueId())) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.invite.already-invited")
        ...
        senderManager.sendMessage(player, messagesFile.getString("party.invite.sender")
                .replace("%target%", target.getName()));
        senderManager.sendMessage(target, messagesFile.getString("party.invite.target")
    ...
    public void removeInvitePlayer(Player player, Player target) {
        ...
        if (!partyData.isInvited(target.getUniqueId())) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.invite.dont-invited")
                    .replace("%player%", player.getName()));
            senderManager.playSound(player, SoundEnum.ERROR);
            return;
        }

        senderManager.sendMessage(target, messagesFile.getString("party.remove-invite")
                .replace("%player%", player.getName()));
    ...
    public void promoteToLeader(Player player, Player target) {
        ...
        if (!userData.isPlayerLeader()) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.no-perms.promote"));
            senderManager.playSound(player, SoundEnum.ERROR);
            return;
        }

        if (!partyData.isInParty(target.getUniqueId())) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.target.not-your-party")
                    .replace("%target%", target.getName()));
            return;
        }

        partyData.setPlayerLeader(target.getUniqueId());
        senderManager.sendMessage(target, messagesFile.getString("party.leader")
                .replace("%player%", player.getName()));
    ...
    public void leaveParty(Player player) {
        ...
        senderManager.sendMessage(player, messagesFile.getString("party.left")
                .replace("%player%", player.getName()));
    ...
    public void kickPlayer(Player player, Player target) {
        ...
        if (!userData.isPlayerLeader()) {
            senderManager.sendMessage(player, messagesFile.getString("party.error.no-perms.kick"));
            senderManager.playSound(player, SoundEnum.ERROR);
            return;
        }
        ...
        senderManager.sendMessage(player, messagesFile.getString("party.kick.sender")
                .replace("%player%", player.getName()));
        senderManager.sendMessage(player, messagesFile.getString("party.kick.target")
