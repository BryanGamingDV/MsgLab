...
public class ChannelCommand implements CommandClass {
    ...
    public boolean joinSubCommand(@Sender Player sender, @OptArg("") String args) {
        ...
        if (formatsFile.getConfigurationSection("channel." + args) == null) {
            senderManager.sendMessage(sender, messagesFile.getString("channel.error.no-exists"));
            senderManager.playSound(sender, SoundEnum.ERROR);
            return true;
        }
        ...
        if (userData.equalsChannelGroup(args)) {
            senderManager.sendMessage(sender, messagesFile.getString("channel.error.joined")
        ...
        senderManager.sendMessage(sender, messagesFile.getString("channel.player.left")
        ...
        senderManager.sendMessage(sender, messagesFile.getString("channel.player.join")
                .replace("%channel%", userData.getChannelGroup()));
    ...
    public boolean quitSubCommand(@Sender Player player) {
        ...
        if (userData.equalsChannelGroup("default")) {
            senderManager.sendMessage(player, messagesFile.getString("channel.error.default.quit"));
            senderManager.playSound(player, SoundEnum.ERROR);
            return true;
        }
        ...
        senderManager.sendMessage(player, messagesFile.getString("channel.player.left")
    ...
    public boolean listSubCommand(@Sender Player player) {
        ...
        senderManager.sendMessage(player, messagesFile.getString("channel.list.message"));
    ...
    public boolean moveSubCommand(@Sender Player sender, @OptArg OfflinePlayer target, @OptArg("") String channel) {
        ...
        if (!senderManager.hasPermission(sender, "channel","move")) {
            senderManager.sendMessage(sender, messagesFile.getString("global-errors.no-perms"));
            senderManager.playSound(sender, SoundEnum.ERROR);
            return true;
        }
        ...
        if (userData.equalsChannelGroup(channel)) {
            senderManager.sendMessage(sender, messagesFile.getString("channel.error.arg2-joined")
        ...
        senderManager.sendMessage(sender, messagesFile.getString("channel.player.move.sender")
                .replace("%arg-2%", target.getName())
        ...
        senderManager.sendMessage(target.getPlayer(), messagesFile.getString("channel.player.move.target")
                .replace("%channel%", channel));
    ...
    public boolean infoSubCommand(@Sender Player sender, @OptArg("") String channel) {
        ...
        if (!senderManager.hasPermission(sender, "channel","info")) {
            senderManager.sendMessage(sender, messagesFile.getString("global-errors.no-perms"));
            senderManager.playSound(sender, SoundEnum.ERROR);
            return true;
        }
        ...
        if (userData.equalsChannelGroup(channel)) {
            senderManager.sendMessage(sender, messagesFile.getString("channel.error.default.info"));
            senderManager.playSound(sender, SoundEnum.ERROR);
            return true;
        }
        ...
        switch (formatsFile.getString("channel." + channel + ".condition.type").toLowerCase()){
            case "group":
                condition = messagesFile.getString("channel.info.condition.perms");
                break;
            case "permission":
                condition = messagesFile.getString("channel.info.condition.group");
                break;
            default:
                return true;
        ...
        if (groupChannel.hasGroupPermission(sender, channel)) {
            status = messagesFile.getString("channel.info.status.yes_permission");
        } else {
            status = messagesFile.getString("channel.info.status.no_permission");
        }

        for (String path : messagesFile.getStringList("channel.info.format")) {
